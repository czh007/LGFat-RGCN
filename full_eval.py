...
def jaccrad(predList, referList):  # terms_reference is the source sentence, terms_model is the candidate sentence
    grams_reference = set(predList)  # De-duplicate; change to list if not needed
    grams_model = set(referList)
    temp = 0
    for i in grams_reference:
        if i in grams_model:
            temp = temp + 1
    fenmu = len(grams_model) + len(grams_reference) - temp  # Concentration
    jaccard_coefficient = temp * 1.0 / fenmu  # Intersections
